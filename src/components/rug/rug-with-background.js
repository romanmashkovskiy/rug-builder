...
class Rug extends Component {
    ...
    async getRoom() {
        var backgroundMesh, backgroundGeometry, backgroundMaterial;
        const textures = {};

        try {
            textures.map = await loadWithPromise(this.props.currentRoomPreset, textureLoader);
            console.log(textures);
        } catch (e) {
            console.log(e);
        }

        backgroundGeometry = new THREE.PlaneGeometry(2, 2, 0);
        backgroundMaterial = new THREE.MeshBasicMaterial(textures);
